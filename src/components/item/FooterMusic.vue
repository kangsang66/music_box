<template>
	<div v-if="playList.songsList[playList.playListIndex]" class="footerMusic">
		<div class="play_left" @click="showSongInfo">
			<img
			...
			<div class="channel">
				<!-- 歌曲名 -->
				<!-- 歌手名 -->
				<span class="songname">{{ playList.songsList[playList.playListIndex].name }}</span>
				<span class="sp">-</span>
				<span class="singername">{{ playList.songsList[playList.playListIndex].ar[0].name }}</span>
			</div>
		</div>
		...
		<audio
			...
			:src="`https://music.163.com/song/media/outer/url?id=${
				playList.songsList[playList.playListIndex].id
			}.mp3`"
		></audio>
	</div>

	<!-- 展示音乐信息 -->
	<van-popup v-model:show="showSong" position="bottom" :style="{ width: '100%', height: '100%' }">
		<div class="SongBox" v-if="playList.songsList[playList.playListIndex]">
			<img
				class="SongBack"
				v-if="playList.songsList[playList.playListIndex].al.picUrl"
				:src="playList.songsList[playList.playListIndex].al.picUrl"
			...
			<div class="SongBox_top">
				<div class="left">
					<van-icon name="arrow-down" @click="showSongInfo" />
				</div>
				<div class="center">
					<!-- 歌曲名 -->
			...
			<div class="SongBox_Center" @click="getLyric">
				<img v-if="!showLyric" :src="playList.songsList[playList.playListIndex].al.picUrl" alt="" />
				<div v-show="!audio.paused && showLyric == false" class="pulseOne"></div>
				<div v-show="!audio.paused && showLyric == false" class="pulseTwo"></div>
				<div v-show="!audio.paused && showLyric == false" class="pulseThree"></div>
				<div class="lyric" ref="musicLyric" :class="{ lyric_active: showLyric }" v-show="showLyric">
					<p
						v-for="(ele, id) in lyric.data"
						:ref="setItemRef"
						:key="id"
						:class="{
							p_active: audio.currentTime * 1000 >= ele.time && audio.currentTime * 1000 < ele.pre,
						}"
					>
						{{ ele.lyc }}
					</p>
			...
			<div class="SongBox_Bottom">
				<div class="top" :class="{ SongBox_top_hidden: showLyric }">
					<img src="../../../public/img/loveHeart.png" alt="" />
					<img src="../../../public/img/download.png" alt="" />
					<img src="../../../public/img/sing.png" alt="" />
					<img src="../../../public/img/pinglun.png" alt="" />
					<img src="../../../public/img/more.png" alt="" />
				</div>
				<div class="center">
				...
				<div class="bottom">
					<img class="one" src="../../../public/img/shunxu.png" alt="" />
					<img @click="before" class="two" src="../../../public/img/next.png" alt="" />
					<img
						@click="play"
						class="three"
						v-if="!playList.isplay"
					...
					<img
						...
						alt=""
					/>
					<img @click="next" class="four" src="../../../public/img/next.png" alt="" />
					<img class="five" src="../../../public/img/songlists.png" alt="" />
				</div>
			</div>
		</div>
	</van-popup>
					...
					if (showLyric.value === true) {
						...
						if (itemRefs.value.offsetTop > 300) {
							...
							musicLyric.value.scrollTo({
								top: itemRefs.value.offsetTop - 300,
								behavior: "smooth",
							});
						}
					}
