		...
		<div v-if="data.showLoginVertify">
			<div class="verificate">
				...
				<div class="verificate-content">
					...
					<span @click="newSend" v-if="!data.VertifyNumber" style="color: darkcyan">重新发送</span>
				</div>
			...
			watch(
				...
				async () => {
					...
					if (values[3].value.length === 1) {
						router.push("/Home");
						// 发送验证请求
						let res = await VerifyPhoneAPI(
							data.PhoneNumber,
							value1.value + value2.value + value3.value + value4.value
						);
						// let result = await GetUserSongListAPI(3985086249);
						// console.log(result);
						// 如果验证成功,就去获取用户的歌单
						if (res.data.code === 200 && res.data.data === true) {
							...
							console.log(res);
							// 登录接口有点问题,暂时不用,直接使用User_id = 3985086249
