<template>
	...
	<div class="search">
		...
		>
			<div class="search_top">
				<van-cell>
					<div class="search_top_inner">
						<svg class="icon" @click="showSearchFn" aria-hidden="true">
			...
			<div class="kinds" v-show="!playList.showSearchPopup && !showSearchResultBox">
				<div>
					<svg
						t="1656751320627"
						class="icon"
				...
				<div>
					...
					>
						<path
							d="M789.74509 0a57.904987 57.904987 0 1 0 57.904987 57.904987A57.904987 57.904987 0 0 0 789.74509 0z"
							fill="#FF517F"
							opacity=".3"
							p-id="23218"
						></path>
						<path
							d="M516.459795 731.604091a185.853685 185.853685 0 1 1 185.689648-185.689648A186.017722 186.017722 0 0 1 516.459795 731.604091z m0-289.524937a103.835289 103.835289 0 1 0 103.671252 103.835289A103.999326 103.999326 0 0 0 516.459795 442.899337z"
							fill="#FF517F"
							p-id="23219"
						></path>
						<path
							d="M516.459795 1023.917654a478.167248 478.167248 0 1 1 0-956.170459 487.517345 487.517345 0 0 1 60.037466 3.772846 41.009198 41.009198 0 1 1-10.170281 82.018396 389.58738 389.58738 0 0 0-49.211038-3.116699 395.656741 395.656741 0 1 0 253.764917 91.860603 41.009198 41.009198 0 1 1 52.491773-62.826091 478.003211 478.003211 0 0 1-306.912837 844.461404z"
							fill="#FF517F"
							p-id="23220"
						></path>
						<path
							d="M661.632356 586.923641a42.813603 42.813603 0 0 1-14.763312-2.788626 41.009198 41.009198 0 0 1-23.457261-52.983884l126.80044-330.042024a41.009198 41.009198 0 0 1 76.605182 29.526622L699.360818 560.677754a41.009198 41.009198 0 0 1-37.728462 26.245887z"
							fill="#FF517F"
							p-id="23221"
						></path>
					</svg>
			...
			>
				<div class="history">
					<span>历史</span>
					<!-- 中划线命名 -->
					<img @click="deleteHistroy" src="../../../public/img/laji.png" alt="" />
					<van-dialog />
				</div>
				<!-- 历史记录 -->
				<div class="historyInfo">
					<div class="historyBox" :ref="filterLi">
						<div>
							<li @click="getName(value)" v-for="(value, id) in historyArr" :key="id">
								{{ value }}
							</li>
							<li @click="MoreHistory" v-show="show" class="arrow">
								<van-icon name="arrow-up" />
							</li>
						</div>
					</div>
					<!-- 展示更多历史记录 -->
					<li @click="MoreHistory" class="arrow" v-show="showMoreHistory">
						<van-icon name="arrow-down" />
					</li>
				</div>
			</div>
			<div class="recommend" v-show="!playList.showSearchPopup && !showSearchResultBox">
				<div class="history">
					<span>推荐</span>
					<!-- 中划线命名 -->
					<svg
						@click="GetRecommendSong"
						t="1658386604833"
						class="icon"
						viewBox="0 0 1024 1024"
						version="1.1"
				...
				<div class="historyInfo">
					<div class="historyBox">
						<div>
							<li @click="getName(value.name)" v-for="(value, id) in data.RecommendSong" :key="id">
			...
			<div class="SearchBox" v-show="playList.showSearchPopup">
				...
				<div class="box">
					...
					>
						<div class="songbox_center">
							...
							<!-- 专辑名 -->
							<span class="album">
								{{ ele.ar[0].name }}<span class="sp">-</span>{{ ele.al.name }}</span
							>
						</div>
				...
				<div class="box">
					...
					<div class="songbox songboxSongsList" v-for="(ele, id) in data.SearchSongsList" :key="id">
						<router-link :to="{ path: '/recommendMusic', query: { id: ele.id } }">
							<img :src="ele.coverImgUrl" alt="" />
							<div class="songbox_right">
								<b>{{ ele.name }}</b>
								<span
									>{{ ele.trackCount }}首,by {{ ele.creator.nickname }}, 播放{{
										ele.playCount
									}}次</span
								>
							</div>
			...
			<div class="showSearchResultBox" v-show="showSearchResultBox">
				<li v-for="(ele, id) in data.SearchSuggestions" :key="id">
					<svg
						t="1658562259689"
						class="icon"
						viewBox="0 0 1024 1024"
					...
					<p @click="getName(`${historyValue} + ${ele.keyword}`)">
						{{ historyValue }}<span>{{ ele.keyword }}</span>
					</p>
				</li>
			</div>
		</van-popup>
				...
				() => {
					...
					if (historyValue.value) {
						...
						timer = setTimeout(async () => {
							...
							// 歌单
							let resSongsList = await SearchSongsListAPI(historyValue.value, 200);
